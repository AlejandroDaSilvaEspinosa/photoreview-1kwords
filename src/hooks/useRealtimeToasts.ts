...
export function useGlobalRealtimeToasts(opts: Options = {}) {
  ...
  const openAction = (sku: string, image?: string) => {
    ...
    return {
      actionLabel: image && onOpenImage ? "Abrir imagen" : (onOpenSku ? "Abrir SKU" : ""),
      onAction: () => {
        if (image && onOpenImage) return onOpenImage(sku, image);
        if (onOpenSku) return onOpenSku(sku);
      }
    };
  };
  ...
  const ensureThreadInfo = async (threadId: number) => {
    ...
    const { data } = await sb.from("review_threads").select("sku,image_name").eq("id", threadId).maybeSingle();
  ...
  useEffect(() => {
    ...
    // precarga cache id -> (sku,image)
    (async () => {
      const { data } = await sb.from("review_threads").select("id,sku,image_name");
      (data || []).forEach((r: any) => {
        threadInfoRef.current.set(r.id, { sku: r.sku, image: r.image_name });
      });
    })();

    const ch = sb.channel("global-review-toasts", { config: { broadcast: { ack: true } } });
    ...
    ch.on("postgres_changes",
      ...
      (payload) => {
        ...
        if (evt === "DELETE") threadInfoRef.current.delete(row.id);
        else threadInfoRef.current.set(row.id, { sku: row.sku, image: row.image_name });

        const title = evt === "INSERT" ? `Nuevo hilo #${row.id}`
        ...
        const description = evt === "DELETE"
          ? `SKU ${row.sku} · ${row.image_name}`
          : `SKU ${row.sku} · ${row.image_name} · Estado: ${statusText}`;

        push({ title, description, variant: evt === "DELETE" ? "error" : variantFromThread(row.status), ...openAction(row.sku, row.image_name) });
    ...
    ch.on("postgres_changes",
      ...
      async (payload) => {
        ...
        // ⚠️ usa is_system (y de paso case-insensitive)
        if ((row as any).is_system) return;
        ...
        const title = evt === "INSERT" ? `Nuevo mensaje en hilo #${row.thread_id}`
        ...
        push({ title, description, variant: evt === "DELETE" ? "error" : "info", ...openAction(info.sku, info.image) });
        notifyNative(title, { body: description });
      }
    );
    ...
    ch.on("postgres_changes",
      ...
      (payload) => {
        ...
        const title = evt === "DELETE" ? `Estado de imagen eliminado` : `Imagen ${row.image_name} actualizada`;
        const statusText = evt === "DELETE" ? "" : (row.status === "needs_correction" ? "Necesita corrección" : "Finalizada");
        const description = evt === "DELETE"
          ? `SKU ${row.sku} · ${row.image_name}`
          : `SKU ${row.sku} · ${row.image_name} · ${statusText}`;

        push({ title, description, variant: evt === "DELETE" ? "error" : variantFromImage(row.status), ...openAction(row.sku, row.image_name) });
        notifyNative(title, { body: description });
      }
    );
    ...
    ch.on("postgres_changes",
      ...
      (payload) => {
        ...
        const title = evt === "DELETE" ? `Estado de SKU eliminado` : `SKU ${row.sku} actualizado`;
        const description = evt === "DELETE"
          ? `Estado eliminado`
          : `Estado: ${labelSkuStatus(row.status)}${row.images_needing_fix != null ? ` · En corrección: ${row.images_needing_fix}` : ""}`;

        push({ title, description, variant: evt === "DELETE" ? "error" : variantFromSku(row.status), ...openAction(row.sku) });
        notifyNative(title, { body: description });
      }
    );
  ...
  // importante: solo depende de `push` (estable) para no re-suscribir cada render
  }, [push]);
}
