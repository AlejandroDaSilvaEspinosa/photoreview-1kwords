...
export function useReviewUrlState() {
  ...
  const setSku = useCallback(
    (sku: string | null, keepImageIfBelongs?: (name: string) => boolean) => {
      const next = new URLSearchParams(searchParams.toString());
      if (sku) {
        next.set("sku", sku);
        const img = next.get("image");
        if (img && keepImageIfBelongs && !keepImageIfBelongs(img))
          next.delete("image");
      } else {
        next.delete("sku");
        next.delete("image");
  ...
  const setThread = useCallback(
    (id: number | null) => {
      const next = new URLSearchParams(searchParams.toString());
      if (id != null) next.set("thread", String(id));
