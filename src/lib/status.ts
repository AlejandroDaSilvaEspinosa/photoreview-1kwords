...
export async function hydrateStatuses(skus: SkuWithImages[]): Promise<SkuWithImagesAndStatus[]> {
  ...
  (imgRows ?? []).forEach(r => imgMap.set(`${r.sku}|${r.image_name}`, r.status as ImageStatus));
  ...
  return skus.map(s => {
    ...
    const counts = images.reduce(
      (acc, i) => { acc[i.status]++; acc.total++; return acc; },
