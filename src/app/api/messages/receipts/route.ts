...
export async function POST(req: NextRequest) {
  ...
  const ids = Array.from(new Set(body?.messageIds || [])).filter((n) => Number.isFinite(n));
  if (!ids.length || !["delivered", "read"].includes(body.mark)) {
    return NextResponse.json({ error: "Bad request" }, { status: 400, headers: res.headers });
  }
  ...
  if (msgsErr) {
    return NextResponse.json({ error: msgsErr.message }, { status: 500, headers: res.headers });
  }
  ...
  if (!targetIds.length) {
    return NextResponse.json({ ok: true, updated: 0 }, { headers: res.headers });
  }
  ...
  if (upErr) {
    return NextResponse.json({ error: upErr.message }, { status: 500, headers: res.headers });
  }
  ...
  if (body.mark === "read") {
    ...
    if (updErr) {
      return NextResponse.json({ error: updErr.message }, { status: 500, headers: res.headers });
    }
  }

  return NextResponse.json({ ok: true, updated: targetIds.length }, { headers: res.headers });
}
